...
function generateCustomerHistory(plateNumber: string): CustomerHistory {
  ...
  const visits = Array.from({ length: Math.floor(Math.random() * 10) + 5 }, (_, i) => {
    const entryTime = new Date(Date.now() - (i + 1) * Math.random() * 30 * 24 * 60 * 60 * 1000);
...
export default function ReportsPage() {
  ...
  const handleCustomerSearch = () => {
    ...
    try {
      ...
      setError("خطا در بارگذاری سوابق مشتری");
    }
  };
